...
describe("SdkCompactionCoordinator", () => {
	...
	it("does not compact a session replaced while waiting for the mutex", async () => {
		const activeSession = makeActiveSession()
		const replacementSession = makeActiveSession({ sessionId: "replacement" })
		const { coordinator, options } = makeCoordinator({ activeSession })
		// Entry check sees the original session; inside the mutex it is replaced.
