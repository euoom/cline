		...
		await this.options.rebuilds.runExclusive(async () => {
			...
			if (active) {
				if (active.sessionId !== taskId) {
					throw new Error("Compaction target changed while waiting to resume the displayed task.")
				}
				if (active.isRunning) {
			...
			if (this.options.sessions.getActiveSession() || this.options.getDisplayedTaskId() !== taskId) {
				throw new Error("Compaction target changed before the displayed task could be resumed.")
			}
