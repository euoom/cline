...
describe("SdkSessionEventCoordinator", () => {
	...
	it("resolves the turn phase to 'error' when the turn surfaced a provider error", async () => {
		// A stream error emits ask:"api_req_failed" via the translator and records the
		// error outcome; the turn-end phase must be "error" so the footer shows
		// Retry / Start New Task instead of the followup input state.
		const { coordinator, options, event } = makeCoordinator({
			translation: {
				messages: [],
				sessionEnded: false,
